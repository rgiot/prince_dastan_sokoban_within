...
{
	TZX_BLOCK *pBlock = TZX_CreateBlock(TZX_PURE_TONE_BLOCK);
	unsigned char *pHeader = TZX_GetBlockHeaderPtr(pBlock);

	if (pHeader!=NULL)
	{
		if (pHeader[0] == TZX_PURE_TONE_BLOCK)
		{
			pHeader[1] = (unsigned char)nLengthOfPulseInTStates;
			pHeader[2] = (unsigned char)(nLengthOfPulseInTStates>>8);
			pHeader[3] = (unsigned char)nPulses;
			pHeader[4] = (unsigned char)(nPulses>>8);
		}
	}
...
{
	TZX_BLOCK *pBlock = TZX_CreateBlock(TZX_PULSE_SEQUENCE_BLOCK);
	unsigned char *pHeader = TZX_GetBlockHeaderPtr(pBlock);
	int i;
	...
	if (pHeader!=NULL)
	{
		if (pHeader[0] == TZX_PULSE_SEQUENCE_BLOCK)
		{
			pHeader[1] = (unsigned char)(nPulses & 0x0ff);
		}
	}
	TZX_AddDataToBlock(pBlock, sizeof(unsigned short)*nPulses);
	pBlockData= TZX_GetBlockDataPtr(pBlock);
	
	for (i=0; i<nPulses; i++)
	{
		int nPulseLength = nPulseLengths[i];
		pBlockData[0] = (unsigned char)(nPulseLength & 0x0ff);
		pBlockData[1] = (unsigned char)((nPulseLength>>8) & 0x0ff);
		pBlockData+=2;
	}
...
{
	unsigned char *pBlockData;
	TZX_BLOCK *pBlock = TZX_CreateBlock(TZX_GROUP_START_BLOCK);
	TZX_AddDataToBlock(pBlock, strlen(sDescription));

	pBlockData= TZX_GetBlockDataPtr(pBlock);
	memcpy(pBlockData, sDescription, strlen(sDescription));
	TZX_AddBlockToEndOfFile(pTZXFile,pBlock);
}
...
int	TZX_GetBlockHeaderSize(unsigned char ID)
{
	switch (ID)
	{
		case TZX_TEXT_DESCRIPTION_BLOCK:
		case TZX_GROUP_START_BLOCK:
			return 1+1;
		case TZX_PURE_TONE_BLOCK:
			return 4+1;
		case TZX_PULSE_SEQUENCE_BLOCK:
			return 1+1;
		case TZX_STANDARD_SPEED_DATA_BLOCK:
			return 4+1;
		case TZX_TURBO_LOADING_DATA_BLOCK:
			return 18+1;
		case TZX_GROUP_END_BLOCK:
			return 1;
...
BOOL TZX_BlockHasData(unsigned char ID)
{
	switch (ID)
	{
		...
		case TZX_DIRECT_RECORDING_BLOCK:
		case TZX_PULSE_SEQUENCE_BLOCK:
		case TZX_CSW_RECORDING_BLOCK:
		case TZX_GROUP_START_BLOCK:
		case TZX_TEXT_DESCRIPTION_BLOCK:
		case TZX_JUMP_TO_BLOCK:
		case TZX_LOOP_START_BLOCK:
			return TRUE;
...
void	TZX_SetBlockSizeInHeader(TZX_BLOCK *pBlock, unsigned long Size)
{
	...
	switch (ID)
	{
		...
		case TZX_GROUP_START_BLOCK:
		{
			pBlockData[0] = (unsigned char)(Size & 0x0ff);
		}
		break;
