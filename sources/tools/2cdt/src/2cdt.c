...
enum
{
    ...
    CPC_METHOD_SPECTRUM,
	CPC_METHOD_2BLOCKS,
	CPC_METHOD_SPEEDLOCK_1985,
};

static int NumFiles = 0;
static const char *Filenames[2];
static const char *sInjectHeaderFilename = 0;
...
#define CPC_TAPE_HEADER_UNALLOCATED 28
...
void WriteStandardSpeedDataBlock(TZX_FILE *pFile, unsigned char SyncPattern, unsigned char *pData, int DataSize, int Pause)
{
	...
	if (pBlock!=NULL)
	{
	    /* one byte for sync, one byte for checksum, one byte for ff to ensure correct pulse */
        int TZX_DataBlockSize = DataSize+2+1;
		...
		if (pBlockData!=NULL)
		{
		    unsigned char CheckSum = SyncPattern;
            int i;
...
void	Speedlock_K7_1985_WriteBlock(TZX_FILE *pFile, const unsigned char *pData, int DataSize)
{
#define SL_ZERO_PULSE_LENGTH 0x0235
#define SL_ONE_PULSE_LENGTH 0x046a
	
	TZX_BLOCK *pBlock;
	unsigned char *pBlockData;
	/* loader defines the expected counts - it also defines what length pulses should follow each other to be valid sequence - these can vary from game to game */
	long64 PulseCounts[4] = {0x0d8,0x0dc,0x0e2, 0x0de}; // 0x01e,0x022,0x028,0x024
	const int nLeaderCycles = 8*4;		// 8 cycles, 4 pulses each	
	int PulseLengths[2] = {0x02cb,0x02cb};
	
	/* the first value is effectively a sync bit */
	/* includes 6 bits for comparison against 0x01d in the loader */
	int PulseLengthsSync[12] = {0x0c4e,0x0c4e,  SL_ONE_PULSE_LENGTH,SL_ONE_PULSE_LENGTH,SL_ONE_PULSE_LENGTH,SL_ONE_PULSE_LENGTH,SL_ONE_PULSE_LENGTH,SL_ONE_PULSE_LENGTH,SL_ZERO_PULSE_LENGTH,SL_ZERO_PULSE_LENGTH,SL_ONE_PULSE_LENGTH,SL_ONE_PULSE_LENGTH};
	unsigned char *pHeader;
	int nPulse;
	char Description[256];
	sprintf(Description,"Speedlock K7 1985 block");
	
	// group start - identify it as speedlock and the version
	TZX_SetupGroupStartBlock(pFile, Description);

	// setup clicking leader - complete 4 cycles - loader syncs on this and checks sequence
	for (nPulse = 0; nPulse<nLeaderCycles; nPulse++)
	{
		TZX_SetupPureTone(pFile, PulseCounts[(nPulse&0x03)], 0x096f);

		TZX_SetupPulseSequence(pFile, sizeof(PulseLengths)/sizeof(PulseLengths[0]), &PulseLengths);
	}

	// important is you get a tone which is next in sequence
	TZX_SetupPureTone(pFile, PulseCounts[0], 0x096f);
	TZX_SetupPulseSequence(pFile, sizeof(PulseLengths)/sizeof(PulseLengths[0]), &PulseLengths);

	// important is you get a tone which is next in sequence
	TZX_SetupPureTone(pFile, PulseCounts[1], 0x096f);

	// then you get the sync bit and initial data
	TZX_SetupPulseSequence(pFile, sizeof(PulseLengthsSync)/sizeof(PulseLengthsSync[0]), &PulseLengthsSync);

	pBlock = TZX_CreateBlock(TZX_PURE_DATA_BLOCK);
	pHeader = TZX_GetBlockHeaderPtr(pBlock);

	
	if (pHeader!=NULL)
	{
		long64 ZeroPulseLength = SL_ZERO_PULSE_LENGTH;
		long64 OnePulseLength = SL_ONE_PULSE_LENGTH;
		long64 Pause = 0x082b;
	...
	if (pBlock!=NULL)
	{
		...
		/* allocate data in block and include checksum byte at end */
		TZX_AddDataToBlock(pBlock, DataSize+1);
		...
		pBlockData[0] = Checksum;
	}
	
	// group start
	pBlock = TZX_CreateBlock(TZX_GROUP_END_BLOCK);
	TZX_AddBlockToEndOfFile(pFile,pBlock);
}
...
void	DisplayInfo()
{
		...
		printf("-i <filename> = Inject file into the unallocated user fields - offset 28\r\n");
}
...
int SetFilenameOptionHandler(ARGUMENT_DATA *pData)
{
	...
	nLength = strlen(opt);
	if (nLength>sizeof(TapeFilename))
		nLength = sizeof(TapeFilename);
	for (i = 0; i<nLength; i++)
	{
...
int SetInjectHeaderOptionHandler(ARGUMENT_DATA *pData)
{
	const char *opt = ArgumentList_GetNext(pData);
	...
	sInjectHeaderFilename = opt;
	printf("File to inject into header: %s\n",  sInjectHeaderFilename);
	return OPTION_OK;
}
...
OPTION OptionTable[]=
{
	...
	{ "l", SetLoadAddressOptionHandler},
	{ "i", SetInjectHeaderOptionHandler},
	{ "?", OutputDetailsOption},
	{NULL, NULL},
};
...
/* clear tape header */
				...
				if (sInjectHeaderFilename!=NULL)
				{
					FILE *fhInject = fopen(sInjectHeaderFilename, "rb");
					if (fhInject!=NULL)
					{
						unsigned char *Buffer = NULL;
						size_t InjectFileLength;
						fseek(fhInject, 0, SEEK_END);
						InjectFileLength = ftell(fhInject);
						fseek(fhInject, 0, SEEK_SET);
						
						if (InjectFileLength!=0)
						{
							Buffer = (unsigned char *)malloc(InjectFileLength);
							if (Buffer!=NULL)
							{
								size_t nUnallocatedCapacity = CPC_TAPE_HEADER_SIZE - CPC_TAPE_HEADER_UNALLOCATED;
								size_t nCopyLength = InjectFileLength;
								if (nCopyLength>nUnallocatedCapacity)
								{
									nCopyLength = nUnallocatedCapacity;
								}
								printf("Copying %d bytes from %s into header at offset %d\n", nCopyLength, sInjectHeaderFilename, CPC_TAPE_HEADER_UNALLOCATED);
								
								fread(Buffer, 1, InjectFileLength, fhInject);
						
								memcpy(&TapeHeader[CPC_TAPE_HEADER_UNALLOCATED], Buffer, nCopyLength);
						...
						{
							printf("File %s\n has 0 length. Can't inject into header\n", sInjectHeaderFilename);
						}
						fclose(fhInject);
					}
					else
				...
				if (CPCMethod == CPC_METHOD_SPEEDLOCK_1985)
				{
					Speedlock_K7_1985_WriteBlock(pTZXFile, &pFileData[FileOffset], FileLengthRemaining);
				}
				else
