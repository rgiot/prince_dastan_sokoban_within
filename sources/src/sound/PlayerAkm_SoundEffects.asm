...
PATCH_SFX_NOISE = 1
PATCH_SFX_FIX_BUG = 1
PATCH_SFX_FIX_BUG_MITIGATION = 1

IFDEF PATCH_SFX_FIX_BUG_MITIGATION
        PATCH_SFX_FIX_BUG = 1
ENDIF
...
PLY_AKM_PtSoundEffectTable: ld hl,0
        ...
        ;Resets the current step, stores the instrument speed.
        ld (hl),0
        inc hl
        ex af,af'
        ld (hl),a
...
PLY_AKM_PSES_ReadFirstByte:
        ...
        ;-------------------------------------------
  IFDEF PATCH_SFX_FIX_BUG_MITIGATION
        ;Get volume set by the song
        ld e,(iy + PLY_AKM_Registers_OffsetVolume)
  ENDIF

        ;Gets the volume.
        call PLY_AKM_PSES_ManageVolumeFromA_Filter4Bits
  IFDEF PATCH_SFX_FIX_BUG_MITIGATION
        ;Get volume set by the SFX
        ld d,a
  ENDIF
  ...
  IFDEF PATCH_SFX_FIX_BUG_MITIGATION
        ...
        ld a,e
        cp d
        jr nc,PLY_AKM_PSES_SavePointerAndExit ; if song volume >= sfx volume, then keep channel open (to eventually hear song note through noise)
  ENDIF
  IFDEF PATCH_SFX_FIX_BUG
...
PLY_AKM_PSES_ReadNoiseAndOpenNoiseChannel:
  IFDEF PATCH_SFX_NOISE
        ; has music already opened noise channel (and set noise level) for any channel?
        ; if yes, do not override music noise level with SFX one
        ; (ideally, channel on which SFX is being played should be ignored...)
        ld a,(PLY_AKM_MixerRegister)
        cpl
        and %00111000
        ...
        ;Opens noise channel.
        res 5,c
        ret nz ; noise was already on at least one channel
